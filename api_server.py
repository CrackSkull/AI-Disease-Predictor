...
ENDPOINTS:
  ...
  POST /predict    → Accepts JSON {lat, lon, age, gender} → returns disease risk % + top_factors
...
def _coerce_optional_positive_age(value: Any) -> Optional[float]:
    if value is None or str(value).strip() == "":
        return None
    try:
        age = float(value)
...
def _prediction_input_payload(
    *,
    ...
    """Build a consistent input payload for prediction and advice flows."""
    return {
        "name": name,
        "age": age,
        "gender": gender,
...
class PredictHandler(BaseHTTPRequestHandler):
    ...
    def _send_error_json(self, status: int, message: str, **extra: Any) -> None:
        """Send a consistent JSON error payload."""
    ...
    def _predict_from_payload(self, body: Dict[str, Any]) -> Dict[str, Any]:
        raw_latitude = body.get("latitude", body.get("lat"))
        raw_longitude = body.get("longitude", body.get("lon"))
        raw_age = body.get("age")
        gender = body.get("gender", "unknown")
        name = body.get("name", "")

        latitude = _coerce_required_float(raw_latitude, "latitude")
        longitude = _coerce_required_float(raw_longitude, "longitude")
        age = _coerce_optional_positive_age(raw_age)
        ...
        return {
            "input": _prediction_input_payload(
                name=str(name),
                age=age,
                gender=str(gender),
                latitude=latitude,
